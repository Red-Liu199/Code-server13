...
import utils
from utils import kl_loss, Loss1
...
class Modal(object):
    ...
    def pretrain(self,posterior=False):
        ...
        epoch_th=0.3*cfg.epoch_num if 'distilgpt2' in cfg.gpt_path else -1
    ...
    def semi_VL(self):
        ...
        for epoch in tqdm(range(epoch_num)):
                        ...
                        if len(dial_batch)==cfg.origin_batch_size or i==len(dial_batch_large)-1:
                            try:
                                ...
                                if cfg.loss_mode_1:
                                    ...
                                    ST_inputs_post=self.get_ST_input(inputs_posterior['contexts_tensor'],\
                                            logits_post,labels_posterior['contexts_tensor'], labels_posterior['contexts_tensor'])
                                    embed_prior=ST_inputs_prior.matmul(self.PrioriModel.get_input_embeddings().weight)
                                    embed_post=ST_inputs_post.matmul(self.PosteriorModel.get_input_embeddings().weight)
                                    outputs1=self.PrioriModel(inputs_embeds=embed_prior)
                                    outputs2=self.PosteriorModel(inputs_embeds=embed_post)
                                    loss_r=self.calculate_loss_and_accuracy(outputs1,labels_prior['contexts_tensor'])
                                
                                    loss_h=self.get_kl_loss(outputs1[0],outputs2[0], bspn_labels_pri['contexts_tensor'],labels_posterior['contexts_tensor'])
                                    loss=loss_r+loss_h
                                else:
            ...
            if cfg.evaluate_during_training:
                ...
                if loss/epoch_step<min_loss1:
                    min_loss1=loss/epoch_step
                    self.save_model()
    ...
    def get_kl_loss(self,logits_pri,logits_post,labels_pri,labels_post):
        ...
        for dial_idx in range(logits_pri.size(0)):
            ...
            while(1):
                ...
                if cfg.loss_mode_1:# log(p1-p2) instead of kl_loss:
                    loss+=Loss1(probs_pri,probs_post.to(probs_pri.device))
                else:
                    loss+=kl_loss(probs_pri,probs_post.to(probs_pri.device))
                count+=bspn_len
                turn_count+=1
                label_pri=label_pri[loc2-loc1:]
                label_post=label_post[loc4-loc3:]
