...
class Modal(object):
    def __init__(self, device):
        ...
        logging.info("model loaded from %s"%cfg.gpt_path)
        if cfg.mode == 'train':
    ...
    def train(self):
        ...
        all_batches = self.reader.get_batches('train')
        if cfg.semi_train:
            all_batches=all_batches[:len(all_batches)//2]
        # compute num_training_steps in get_batches()
        optimizer, scheduler = self.get_optimizers()
        ...
        for epoch in tqdm(range(cfg.epoch_num)):
            ...
            for batch_idx, dial_batch in enumerate(data_iterator):
                inputs, masked_labels = self.reader.convert_batch_session(dial_batch)
                try:  # avoid OOM
                    self.model.train()
                    if log_inputs > 0:  # log inputs for the very first two turns
                        self.log_first_inputs(inputs)
                    ...
                    # to tensor
                    inputs = self.add_torch_input(inputs)
                    masked_labels=self.add_torch_input(masked_labels)
                    # loss
                    outputs = self.model(inputs['contexts_tensor'])
                    # outputs = self.model(inputs['contexts_tensor']) # debugging with GPT2Model
                    labels=masked_labels['contexts_tensor'] if cfg.label_mask else inputs['contexts_tensor']
                    loss = self.calculate_loss_and_accuracy(
            ...
            logging.info('Train epoch time: {:.2f} min, epoch loss: {:.4f}'.format(
                (time.time()-btm)/60, tr_loss))
            # save model after every epoch
            # 修改记录：更改了模型存储的条件, 在同一个checkpoint存储最小损失的模型
            logging.info('epoch:{}, loss:{:.4f}'.format(epoch, tr_loss/epoch_step))
            if tr_loss/epoch_step<min_loss:
                min_loss=tr_loss/epoch_step
                self.save_model(0, 0)#0，0存储损失最小模型
            
            if tr_loss/epoch_step>prev_loss:#触发早停
                cfg.early_stop_count-=1
                logging.info('early stop count=%d'%cfg.early_stop_count)
                if cfg.early_stop_count==0:
                    break

            if tr_loss/epoch_step<0.6 and cfg.evaluate_during_training:
                #每个损失小于0.6的都简单评估一下，防止模型在训练集上过拟合
                eval_result=self.validate()
                if eval_result['score']>max_score:
                    max_score=eval_result['score']
                    self.save_model(0,1)#1，1存储评估最好模型
            prev_loss=tr_loss/epoch_step
    ...
    def validate(self, data='dev', do_test=False):
        ...
        set_stats = self.reader.set_stats[data]
        logging.info("***** Running Evaluation on %s *****" %data)
        logging.info("  Num Turns = %d", set_stats['num_turns'])
        # logging.info("  Num Dialogs = %d", set_stats['num_dials'])
        ...
        btm = time.time()
        result_collection = {}
        #修改记录：如果在训练时评估，那么只评估100个样本
        if cfg.evaluate_during_training:
            eval_data=eval_data[:100]
        log_inputs=0 if cfg.evaluate_during_training else 5
        with torch.no_grad():
            for dial_idx, dialog in enumerate(tqdm(eval_data)):
                ...
                for turn_idx, turn in enumerate(dialog):
                    ...
                    inputs = self.reader.convert_turn_eval(
                        turn, pv_turn, first_turn)
                    if log_inputs > 0:
                        logging.info(self.tokenizer.decode(inputs['context']))
                        log_inputs -= 1
                    inputs = self.add_torch_input_eval(inputs)
                    ...
                    if cfg.use_true_curr_bspn: # generate act, response
                        ...
                        #修改记录，增加了下方的输出语句
                        #if log_inputs>0:
                        #    logging.info(self.tokenizer.decode(inputs['context'][:-1] + bspn_gen + db))
                        outputs_db = self.model.generate(input_ids=inputs['context_tensor_db'],
                                                    max_length=context_length+80, temperature=0.7,  #top_p=0.9, num_beams=4,
                                                    pad_token_id=self.tokenizer.eos_token_id, eos_token_id=self.tokenizer.encode(['<eos_r>'])[0])
                        generated_ar = outputs_db[0].cpu().numpy().tolist()
                    ...
                    pv_turn['resp'] = turn['resp'] if cfg.use_true_prev_resp else decoded['resp']
                    pv_turn['bspn'] = turn['bspn'] if cfg.use_true_prev_bspn else decoded['bspn']
                    #修改记录：源代码中db最后一个token是sos_a，这与aspn的第一个token重复，因此删去
                    pv_turn['db'] = turn['db'] if cfg.use_true_curr_bspn else db[:-1]
                    pv_turn['aspn'] = turn['aspn'] if cfg.use_true_prev_aspn else decoded['aspn']
        ...
        if len(cfg.eval_load_path.split('/'))>2:
