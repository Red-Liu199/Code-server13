...
class Modal(object):
    def __init__(self, device=[0]):
        ...
        if cfg.data_aug and cfg.mode=='selftrain':#self training cannot generate accurate database result 
            cfg.dbs={
        ...
        # create model: gpt2
        single_mode=['pretrain','train','selftrain','test_pos']
        ...
        elif cfg.mode=='semi_train':#semi-VL
    ...
    def convert_eval_batch(self,data,contexts, turn_num,bs_gen,prior=False,db_gen=None,resp_gen=None,aspn_gen=None, gen_db=False):
        ...
        if gen_db:#在使用后验网络生成数据库结果时使用
            ...
            for id,context in enumerate(contexts):
                if turn_num==0:
                    if prior:
                        if db_gen is None: # db_gen is None indicates that bspn hasn't been generated
                            contexts[id]=data[id][turn_num]['user']+[self.sos_b_id]
                        # if you need to set model_act to False, then you need to rewrite the following code
                        elif aspn_gen is None:
                            contexts[id]=context[:-1]+bs_gen[id]+db_gen[id]+[self.sos_a_id]
                        else:
                            contexts[id] = context[:-1] + aspn_gen[id] + [self.sos_r_id]
                    else:
                        if db_gen is None:
    ...
    def get_aspn(self,aspn_tensor):
        ...
        for i ,aspn in enumerate(aspn_batch):
            ...
            if aspn.count(sos_a_id)>1:
                ...
                aspn=aspn[-last:]
            aspn_text=self.tokenizer.decode(aspn)
            if cfg.sys_act_ctrl:
                aspn_fix= '<sos_a> ' + self.reader.fix_aspn(aspn_text) + ' <eos_a>'
                aspn=self.reader.modified_encode(aspn_fix)
    ...
    def validate_fast(self,data='dev'):
        ...
        if cfg.mode=='pretrain' or cfg.mode=='train' or cfg.mode=='selftrain':
        ...
        for batch in batches:
            try:
                if batch==[]:
                    continue
                if cfg.test_grad:
                    batch=self.generate_batch_with_grad(batch)
                else:
    ...
    def generate_batch(self, batch):
        ...
        with torch.no_grad():
            for turn_num in range(len(batch[0])):
                ...
                if not cfg.use_true_curr_bspn: # generate bspn
                ...
                for i in range(resp_max_len):
                    ...
                    preds=outputs.logits[:,-1,:].argmax(-1)#B
                    if i==0:
                        act_tensor=preds.unsqueeze(1)
                    else:
                        act_tensor=torch.cat([act_tensor,preds.unsqueeze(1)],dim=1)
                    attentions=torch.cat((attentions,torch.ones(batch_size,1).long().to(self.device1)),dim=1)
                    inputs=preds.unsqueeze(1)
                    end_flag+=(preds.cpu().numpy()==eos_a_id).astype(float)
                    if sum(end_flag==0)==0:
                        break
                act_gen=self.get_aspn(act_tensor)

                # generate response
                past_key_values=None
                end_flag=np.zeros(len(batch))
                contexts=self.convert_eval_batch(batch,contexts,turn_num,bs_gen,\
                    prior=True,db_gen=db_gen, aspn_gen=act_gen)
                inputs,attentions=self.batch_align(contexts,left_len=resp_max_len,return_attn=True)
                inputs=torch.tensor(inputs).to(self.device1)#B,T
                attentions=torch.tensor(attentions).to(self.device1)
                for i in range(resp_max_len):
                ...
                for i in range(len(batch)):
                    ...
                    #batch[i][turn_num]['db']=db_gen[i]
                    if cfg.model_act:
                        batch[i][turn_num]['aspn_gen']=act_gen[i]
    ...
    def generate_batch_origin(self,batch):
        bs_max_len=75
            ...
            if cfg.exp_no=='':
                if cfg.mode=='pretrain':
                    if cfg.posterior_train:
                        ...
                        cfg.exp_no='pre_'
                elif cfg.mode=='selftrain':
                    cfg.exp_no='ST_'
                    if cfg.fix_ST:
                        cfg.exp_no=cfg.exp_no+'fix_'
                elif cfg.mode=='semi_train':
                    cfg.exp_no='VL_'
                elif cfg.mode=='train':
                    cfg.exp_no='full'
                    if cfg.posterior_train:
                ...
                if cfg.data_aug:
                    cfg.exp_no='full_aug_VL' if cfg.mode=='semi_train' else 'full_aug_ST'
            print('exp_no:',cfg.exp_no)
