...
class Modal(object):
    def __init__(self, device=[0]):
        ...
        if not cfg.combine_eval:
            ...
            self.nlg_model=GPT2LMHeadModel.from_pretrained(cfg.gpt_path2)
    ...
    def pretrain_turn_level(self, mode='nlg'):
        ...
        for epoch in range(cfg.epoch_num):
            ...
            for batch_idx, dial_batch in enumerate(data_iterator):
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    if mode=='nlg':
                        inputs, labels = self.convert_batch_turn(turn_batch, pv_batch, first_turn)
                    elif mode=='dst':
                        inputs, labels = self.convert_batch_turn_dst(turn_batch, pv_batch, first_turn)
                    pv_batch = self.get_pv_batch(pv_batch, turn_batch)
                    try:  # avoid OOM
                        self.model.train()
                        if log_inputs > 0:  # log inputs for the very first two turns
                            logging.info('Input examples:')
                            logging.info(self.tokenizer.decode(inputs['contexts'][3]))
                            log_inputs-=1
            ...
            if epoch>epoch_th:
                metrics=self.eval(data='dev', mode=mode)
                if mode=='nlg':
    ...
    def convert_batch_turn(self, turn_batch, pv_batch, first_turn=False, mode='train', db_batch=None):
        ...
        label_contexts = []
        if db_batch is None:
            db_batch=turn_batch['db']
        if first_turn:
            batch_zipped = zip(db_batch, turn_batch['user'], turn_batch['resp'])
                
            for db, u, r in batch_zipped:
                if mode=='train':
                    context = db+u+r
                    label_context=len(db+u)*[cfg.pad_id]+r
                    label_contexts.append(label_context)
                else:
                    context=db+u+[self.sos_r_id]
                contexts.append(context)
        else:
            batch_zipped = zip(db_batch, pv_batch, turn_batch['user'], turn_batch['resp'])        
            for db, h, u, r in batch_zipped:
                if mode=='train':
                    context = db+h+u+r
                    label_context=len(db+h+u)*[cfg.pad_id]+r
                    label_contexts.append(label_context)
                else:
                    context=db+h+u+[self.sos_r_id]
                contexts.append(context)
        if mode=='train':
    ...
    def convert_batch_turn_dst(self, turn_batch, pv_batch, first_turn=False, mode='train'):
        assert mode in ['train', 'test']
        inputs = {}
        labels = {}
        contexts = []
        ...
        if first_turn:
            ...
            for u, r in batch_zipped:
                if mode=='train':
                    context = u+r
                    label_context=len(u)*[cfg.pad_id]+r
                    label_contexts.append(label_context)
                else:
                    context=u+[self.sos_b_id]
                contexts.append(context)
        else:
            batch_zipped = zip(pv_batch, turn_batch['user'], turn_batch['bspn'])        
            for h, u, r in batch_zipped:
                if mode=='train':
                    context = h+u+r
                    label_context=len(h+u)*[cfg.pad_id]+r
                    label_contexts.append(label_context)
                else:
                    context=h+u+[self.sos_b_id]
                contexts.append(context)
        if mode=='train':
            inputs['contexts'] = contexts
            inputs['contexts_np'], inputs['lengths'] = utils.padSeqs_gpt(inputs['contexts'], cfg.pad_id)
    ...
    def get_pv_batch(self, pv_batch, turn_batch):
        ...
        if pv_batch is None:# first turn
            ...
            for h, u, r in zip(pv_batch,turn_batch['user'], turn_batch['resp']):
                new_pv_batch.append(h+u+r)
        return new_pv_batch

    def eval(self, data='dev', mode='nlg'):
        batch_size0=cfg.batch_size
        ...
        with torch.no_grad():
            for dial_batch in data_iterator:
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    first_turn = (turn_num == 0)
                    if mode=='nlg':
                        contexts = self.convert_batch_turn(turn_batch, pv_batch, first_turn, mode='test')
                    elif mode=='dst':
                        contexts = self.convert_batch_turn_dst(turn_batch, pv_batch, first_turn, mode='test')
                    pv_batch = self.get_pv_batch(pv_batch, turn_batch)
                    if self.global_output>0:
                        self.global_output-=1
                        logging.info('Evaluation inputs:{}'.format(self.tokenizer.decode(contexts[0])))
                    gen_batch=self.generate_batch(self.model, contexts, 60, eos_id)
                    gen_resp=self.convert_batch_ids_to_tokens(self.tokenizer, gen_batch, sos_id, eos_id)
                    gen+=gen_resp
                    if mode=='nlg':
                        oracle+=self.convert_batch_ids_to_tokens(self.tokenizer, turn_batch['resp'], sos_id, eos_id)
                    elif mode=='dst':
                        oracle+=self.convert_batch_ids_to_tokens(self.tokenizer, turn_batch['bspn'], sos_id, eos_id)
        metrics=self.evaluator.calculate_metrics(gen, oracle, modular=mode)
        if mode=='nlg':
            logging.info('Validation BLEU score:{:.2f}'.format(metrics))
        elif mode=='dst':
            logging.info('Validation joint accuracy:{:.3f}, P/R/F1:{}'.format(metrics[0], metrics[1]))
        cfg.batch_size=batch_size0 #restore to the original batch size
        return metrics
        ...
        with torch.no_grad():
            for batch in batches:
                ...
                for turn_num, turn_batch in enumerate(batch):
                    ...
                    contexts=self.convert_batch_turn(turn_batch, pv_batch, first_turn, mode='test', db_batch=db_batch_ids)
                    ...
                    if log_output>0:
                        log_output-=1
                        logging.info('Context:{}\nGen_resp:{}'.format(self.tokenizer.decode(contexts[3]), gen_resp[3]))
...
def main():
    ...
    if args.mode =='pretrain' or args.mode=='train':
        m.pretrain_turn_level(mode=cfg.modular)
