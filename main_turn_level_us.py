...
class Modal(object):
    ...
    def pretrain_turn_level(self, posterior=False):
        if cfg.mode=='train':
            ...
            if not os.path.exists(cfg.divided_path):
                ...
                if cfg.train_us:
                    temp_path=os.path.join(cfg.data_path,'divided_us_data{}.json'.format(cfg.spv_proportion-5))
                else:
        ...
        for epoch in range(cfg.epoch_num):
            ...
            for batch_idx, batch0 in enumerate(all_batches):
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    inputs, labels = self.reader.convert_batch_turn(turn_batch, pv_batch, first_turn, side=side, posterior=posterior)
                    if cfg.train_us:
                        pv_batch = self.reader.get_pv_batch(pv_batch, resp=turn_batch['resp'], 
                            aspn=turn_batch['sys_act'], side='user') 
                    else:
                ...
                if cfg.save_type=='min_loss':
                    ...
                    if posterior:
                        ...
                        if cfg.train_us:
                            ...
                            # we define the final score = BLEU + 25*ActF1
                            score=F1*25+bleu
                        else:
    ...
    def semi_VL_US(self):
        if cfg.VL_ablation:
            logging.info('------Running ablation experiments of semi-VL------')
        else:
            logging.info('------Running variational learning for user simulator------')
        ...
        for epoch in range(cfg.epoch_num):
            ...
            for batch_idx, dial_batch_dict in enumerate(all_batches):
                pv_goal_batch=None
                for turn_num, turn_batch in enumerate(dial_batch_dict['batch'][::-1]):
                    if dial_batch_dict['supervised']==False: # use the inference model to generate pseudo labels
                        turn_batch=self.gen_hidden_state(turn_batch)
                        goal_batch, gpan_batch=self.infer_goal(pv_goal_batch, turn_batch['usr_act'], mode='id')
                        turn_batch['goal']=gpan_batch
                        pv_goal_batch=goal_batch
                    pv_batch=None if turn_num==len(dial_batch_dict['batch'])-1 else dial_batch_dict['batch'][-turn_num-2]['resp']
                    first_turn = (turn_num == len(dial_batch_dict['batch'])-1)
                    ...
                    for i, batch in enumerate(mini_batches):
                        mini_pv_batch=None if not mini_pv_batches else mini_pv_batches[i]
                        if not dial_batch_dict['supervised'] and not cfg.VL_ablation:
                            ...
                            inputs_prior, labels_prior = self.reader.convert_batch_turn(batch, mini_pv_batch, first_turn, posterior=False, side='user')
                            inputs_posterior, labels_posterior = self.reader.convert_batch_turn(batch, mini_pv_batch, first_turn, posterior=True, side='user')
    ...
    def infer_goal(self, pv_goal_batch, user_act_batch, mode='id'):
        # mode: id or token
        # pv_goal_batch: dict
        # user_act_batch: sequence
        goal_batch=[]
        gpan_batch=[]
        pv_goal_batch=[None for _ in range(len(user_act_batch))] if pv_goal_batch is None else pv_goal_batch
        for pv_goal, user_act in zip(pv_goal_batch, user_act_batch):
            #pv_goal_t=self.tokenizer.decode(pv_goal) if mode=='id' else pv_goal
            user_act_t=self.tokenizer.decode(user_act) if mode=='id' else user_act
            user_act_d=self.reader.aspan_to_act_dict(user_act_t, side='user')
            goal=self.reader.accumulate_goal(pv_goal, user_act_d)
            gpan=self.tokenizer.encode('<sos_g> '+self.reader.goal_to_gpan(goal)+' <eos_g>')
            gpan_batch.append(gpan)
            goal_batch.append(goal)
        return goal_batch, gpan_batch

    def gen_hidden_state(self, turn_batch):
        self.model=self.PosteriorModel
        self.model.eval()
        max_len_a=20
        with torch.no_grad():
            # generate aspn
            contexts=self.convert_eval_batch_turn(turn_batch)
            aspn_batch=self.generate_batch(self.model, contexts, max_len_a, self.eos_ua_id)
            aspn_gen=self.get_aspn(aspn_batch, side='user')
            turn_batch['usr_act']=aspn_gen
        return turn_batch
    
    def convert_eval_batch_turn(self, turn_batch):
        eval_batch=[]
        for u in turn_batch['user']:
            context=u+[self.sos_ua_id]
    ...
    def get_aspn(self,aspn_tensor, side='sys'):
        ...
        aspn_gen=[]
        eos_a_id=self.eos_a_id if side=='sys' else self.eos_ua_id
        sos_a_id=self.sos_a_id if side=='sys' else self.sos_ua_id
    ...
    def generate_batch_us(self, batch):
        ...
        with torch.no_grad():
            for turn_num, turn_batch in enumerate(batch):
                # update goal if strict evaluation
                if cfg.strict_eval and turn_num>0:
                    turn_batch['goal0']=turn_batch['goal'] # save original goal
                    for j in range(batch_size):
                        pv_goal=self.reader.aspan_to_act_dict(self.tokenizer.decode(batch[turn_num-1]['goal'][j]), side='user')
                        pv_user_act=self.reader.aspan_to_act_dict(self.tokenizer.decode(batch[turn_num-1]['usr_act_gen'][j]), side='user')
                        pv_sys_act=self.reader.aspan_to_act_dict(self.tokenizer.decode(aspn_gen[j]), side='sys') if cfg.user_nlu else None
                        goal, _=self.reader.update_goal(pv_goal, pv_user_act, sys_act=pv_sys_act)
                        turn_batch['goal'][j]=self.tokenizer.encode('<sos_g> '+self.reader.goal_to_gpan(goal)+' <eos_g>')
                ...
                # we generate aspn according to resp, if US has NLU module
                if cfg.user_nlu and cfg.strict_eval:
                    contexts=self.reader.convert_eval_batch_turn_us(turn_batch, pv_batch=turn_batch, nlu=True)
                    aspn_batch=self.generate_batch(self.model, contexts, max_len=20, eos_id=self.eos_a_id)
                    aspn_gen=self.get_aspn(aspn_batch)
                    turn_batch['sys_act_gen']=aspn_gen
                    pv_batch=self.reader.get_pv_batch(pv_batch, resp=turn_batch['resp'], aspn=aspn_gen, side='user')
                else:
    ...
    def validate_us(self, data='dev'):
        ...
        logging.info('BLEU:{:.2f}, Avg_Precious:{:.3f}, Avg_Recall:{:.3f}, Avg_F1:{:.3f}'.format(bleu, P, R, F1))
...
def main():
    ...
    if cfg.train_us:
        cfg.divided_path=os.path.join(cfg.data_path,'divided_us_data{}.json'.format(cfg.spv_proportion))
    else:
